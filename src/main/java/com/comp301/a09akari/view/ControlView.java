...
public class ControlView implements FXComponent, ModelObserver {
    ...
    public Parent render() {
        ...
        if(prev == null){
            ...
            prev.setOnAction(
                    (ActionEvent event) -> controller.clickPrevPuzzle()
            );
        }
        if(next == null){
            next = new Button("Next puzzle");
            next.setOnAction(
                    (ActionEvent event) -> controller.clickNextPuzzle()
            );
        }
        if(rand == null){
            rand = new Button("Random puzzle");
            rand.setOnAction(
                    (ActionEvent event) -> controller.clickRandPuzzle()
            );
        }
        if(reset == null){
            reset = new Button("Reset puzzle");
            reset.setOnAction(
                    (ActionEvent event) -> controller.clickResetPuzzle()
            );
        }
